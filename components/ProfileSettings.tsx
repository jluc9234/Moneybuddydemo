...
export default function ProfileSettings({ user, onLogout }: ProfileSettingsProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If you store names in metadata instead, you might need supabase.auth.updateUser({ data: { full_name: newName } })
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            autoComplete="email"
            // disabled property removed here so you can edit it
            className="w-full bg-gray-700 border border-gray-600 rounded-md p-3 focus:ring-lime-400 focus:border-lime-400 transition"
          />
        </div>
